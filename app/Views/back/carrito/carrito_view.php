			...
			<table class="table table-hover table-dark table-responsive-md">
				...
				<tbody>
					...
					<?php foreach ($cart as $item): ?>
						...
						<input type="hidden" name="cart[<?= esc($item['rowid']) ?>][imagen]" value="<?= esc($item['options']['imagen']) ?>">
						<tr class="table-danger align-middle">
  							<td><img src="<?= base_url('assets/uploads/' . $item['options']['imagen']) ?>" width="80" height="80" alt="<?= esc($item['name']) ?>"></td>
  							<td><?= esc($item['name']) ?></td>
        			...
        			<tr class="table-light">
          				...
          				<td colspan="2" class="text-end">
            				<input type="button" class="btn btn-primary btn-lg" value="Borrar Carrito" onclick="window.location='<?= base_url('borrar') ?>'">
           					<input type="button" class="btn btn-secondary btn-lg" value="Comprar" onclick="window.location='<?= base_url('carrito_comprar') ?>'">
          				</td>
        			</tr>
      			</tbody>
    		</table>
